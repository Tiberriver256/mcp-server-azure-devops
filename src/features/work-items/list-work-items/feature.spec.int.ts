...
describe('listWorkItems integration', () => {
  ...
  beforeAll(async () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      try {
        const workItem = await createWorkItem(connection, projectName, 'Task', options);
  ...
  test('should list work items with custom WIQL query', async () => {
    ...
    if (shouldSkipIntegrationTest() || !connection || createdWorkItemIds.length === 0) {
      return;
    }
