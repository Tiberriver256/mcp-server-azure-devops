...
describe('Azure DevOps Errors', () => {
  ...
  describe('isAzureDevOpsError', () => {
    it('should return true for Azure DevOps errors', () => {
      ...
      expect(
        isAzureDevOpsError(new AzureDevOpsAuthenticationError('Test')),
      ).toBe(true);
      expect(
        isAzureDevOpsError(new AzureDevOpsValidationError('Test', undefined)),
      ).toBe(true);
      expect(
      ...
      expect(
        isAzureDevOpsError(new AzureDevOpsRateLimitError('Test', new Date(0))),
      ).toBe(true);
    });
  ...
  describe('formatAzureDevOpsError', () => {
    ...
    it('should format non-Azure DevOps error', () => {
      ...
      expect(formatAzureDevOpsError(error as AzureDevOpsError)).toBe(
        'Error: Regular error',
      );
    });
    ...
    it('should handle objects with missing properties', () => {
      ...
      expect(formatAzureDevOpsError(errorWithoutMessage)).toBe(
        'NoMessage: Unknown error',
      );
      expect(formatAzureDevOpsError(emptyObject)).toBe(
